...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, variant, size, asChild = false, render, nativeButton = false, ...props }, ref) => {
    if (render) {
        ...
        return React.cloneElement(render as React.ReactElement<any>, {
            className: cn(buttonVariants({ variant, size, className }), renderProps.className as string | undefined),
            children: props.children,
        })
    }
    const Comp = asChild && !nativeButton ? Slot : 'button'
    return (
        <Comp
