...
- Aerobase Concierge VPS demo: a separate public-safe infrastructure case study showing how an isolated OpenClaw travel-agent runtime can be provisioned behind VPS, DNS, cloud-init, health checks, and event streaming. The product framing covers booking and arrangement management for air travel, price research, award pricing and best-redemption strategy, jetlag recovery scheduling, calendar coordination, and knowledge-graph-backed recommendations across thousands of travel entity pages. The public version uses dry-run Hetzner and Cloudflare adapters, redacted runtime bootstrap config, fixture travel-agent events, and no private git history.
...
- Loungebase: a standalone portfolio case study for an iOS airport lounge companion that combines SwiftUI mobile UX, travel content quality, subscription readiness, App Store submission prep, privacy packaging, and fallback design. It is intentionally included as documentation only, without source code, signing assets, App Store Connect identifiers, RevenueCat products, backend refs, reviewer credentials, or build artifacts.
